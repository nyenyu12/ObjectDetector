...
import com.example.boazandelad.canifindit.MainActivity.Companion.SERVER_IP
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE)
            ...
            findViewById<TextView>(R.id.image_desc).text= "Premission not granted immidietly"
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == PICK_IMAGE){
            ...
            findViewById<TextView>(R.id.image_desc).text= "Image not processed yet"
        }
    }


    fun sendImage (view: View) = try {
        ...
        val Job = GlobalScope.launch {
            try {
                val url:String = "http://${findViewById<EditText>(R.id.IPText).text}:${findViewById<EditText>(R.id.PortText).text}"
                findViewById<TextView>(R.id.image_desc).text = "Connecting to " + url;
                response = post(url, files = listOf(File(path).fileLike(name = "Image.jpg")))
